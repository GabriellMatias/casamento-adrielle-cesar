const QR_FILLED = new Set([0, 1, 3, 4, 5, 6, 7, 9, 11, 12, 13, 17, 20, 21, 23, 24]);
const QR_CELLS = Array.from({ length: 25 }, (_, i) => ({ i, filled: QR_FILLED.has(i) }));

export default function QrCodePlaceholder({ label = "Acesse o cardápio digital" }: { readonly label?: string }) {
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      >
        ...
        <div className="grid grid-cols-5 grid-rows-5 gap-0.5 w-16 h-16 opacity-30" aria-hidden="true">
          {QR_CELLS.map(({ i, filled }) => (
            <div
              key={i}
              className={`rounded-[1px] ${filled ? "bg-gold" : "bg-transparent"}`}
            />
          ))}
        </div>
      </div>
